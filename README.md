...
- **아키텍처 리팩토링 및 UX 고도화**:
  - `App.jsx`가 비대해지는 문제를 해결하기 위해 로그인 로직(`useAuth.js`), 데이터 연동 로직(`useNotes.js`)을 커스텀 훅으로 분리.
  - UI 컴포넌트를 분리하고(`NoteCard.jsx`, `SkeletonLoader.jsx`), 기존 `window.prompt`를 사용하던 텍스트 수정 및 탭 추가 기능을 인라인 에디팅(Inline Editing) 방식으로 개편하여 사용자 경험(UX) 개선.
  - 렌더링 성능 최적화를 위해 카테고리 필터링 로직에 `useMemo` 적용.
  - 데이터 로딩 시 화면 깜빡임을 방지하는 스켈레톤(Skeleton) UI 도입.

## 🚀 다음 작업자를 위한 가이드 (Next Steps)
- **현재 구조**: 비즈니스 로직은 `src/hooks/` 폴더에, UI 컴포넌트는 `src/components/` 폴더로 분리되어 있으며 `src/App.jsx`에서 이들을 조합하여 사용하고 있습니다. CSS는 `src/App.css`, `src/index.css`, `src/components_styles.css`로 나뉘어 있습니다.
- **오프라인 지원**: 현재 모바일 환경 등에서 인터넷 연결이 끊겼을 때도 메모를 볼 수 있도록 Firebase Firestore의 Offline Persistence 기능을 도입하는 것을 검토 중입니다 (`src/firebase.js`에 설정 가능).
...
- **보안 규칙 강화**: Firebase 콘솔에서 Firestore Security Rules를 확인하고, 반드시 요청자의 `uid`와 문서의 `uid`가 일치할 때만 접근(`read`, `write`)을 허용하도록 설정이 필요합니다.
